  ...
  eufs_msgs__msg__WaypointArrayStamped__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    eufs_msgs__msg__WaypointArrayStamped * data =
      (eufs_msgs__msg__WaypointArrayStamped *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!eufs_msgs__msg__WaypointArrayStamped__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          eufs_msgs__msg__WaypointArrayStamped__fini(&output->data[i]);
        }
        return false;
      }
    }
